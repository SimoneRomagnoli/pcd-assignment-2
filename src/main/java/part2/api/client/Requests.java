...
public class Requests {
    ...
    private static final String REQUEST_URI_GET_TRAIN_DETAILS = "/msite/api/solutions/";
    ...
    public Promise<JsonArray> getRequestForSolutionDetail(String solutionId) {
        Promise<JsonArray> result = Promise.promise();
        try {
            this.session
                    .get(HTTPS_PORT, HOST_GET_TRAIN_SOLUTIONS, REQUEST_URI_GET_TRAIN_DETAILS+solutionId+"/info")
                    .ssl(true)
                    .send()
                    .onSuccess(response -> {
                        //System.out.println("Received response with status code " + response.statusCode());
